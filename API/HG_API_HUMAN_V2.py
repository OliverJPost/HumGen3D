...
class HG_Human():
    ...
    def set_height(self, context=None, height: float = None):
        ...
        if height < 150 or height > 200:
            raise ValueError(f"Height must be between 150 and 200 cm, input value was: {height}")
    ...
    def set_face_hair(self, context=None, chosen_face_hair_option = None):
        ...
        if not chosen_face_hair_option:
            chosen_face_hair_option = random.choice(self.get_facial_hair_options(context))

        self.__set_active_in_pcoll(context, 'face_hair', chosen_face_hair_option)
        self.set_hair_visibility(False)
