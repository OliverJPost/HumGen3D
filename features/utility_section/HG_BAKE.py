...
class HG_BAKE(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if self.finish_modal:
            ...
            human_name, texture_name, bake_obj, mat_slot, tex_type = [
                v for _, v in bake_enum_dict.items()
            ]
            hg_rig = bpy.data.objects.get(human_name)
            hg_log(
                f"Started baking object {bake_obj.name}, texture {texture_name}",
                level="DEBUG",
            )
...
def bake_texture(
    ...
    image = bpy.data.images.new(
        naming + f"_{bake_type.lower()}", width=resolution, height=resolution
    )
