...
class HG_Key_Blocks:
    ...
    def __get_key_blocks_from_prefix(self, prefix):
        return [
            sk
            for sk in self._body_object.data.shape_keys.key_blocks
...
class HG_Human:
    ...
    def __check_if_valid_hg_rig(self, hg_rig):
        ...
        if not hg_rig:
            raise TypeError(
                'Expected a Blender object of type "ARMATURE", not "NoneType"'
            )

        if not hg_rig.HG.ishuman:
            if hg_rig.type != "ARMATURE":
                raise TypeError(
                    f'Expected a human object of type "ARMATURE", not "{hg_rig.type}"'
                )
            else:
                raise ValueError(
    ...
    def __check_if_in_creation_phase(self):
        ...
        if not _in_creation_phase(self._rig_object):
            raise HumGenException(
                "The human needs to be in creation phase to use this method."
            )
    ...
    def __check_if_in_finalize_phase(self):
        ...
        if _in_creation_phase(self._rig_object):
            raise HumGenException(
                "The human needs to be in finalize phase to use this method."
            )
