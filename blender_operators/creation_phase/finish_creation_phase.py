...
from .length import apply_armature, apply_length_to_rig
...
def _remove_unused_eyebrows(hg_body):
    ...
    remove_list = [
        mod.particle_system.name for mod in eyebrows if not mod.show_render
    ]
...
def _create_backup_human(context, hg_rig):
    ...
    for obj in hg_rig.children:
        ...
        add_to_collection(
            context, obj_copy, collection_name="HumGen_Backup [Don't Delete]"
        )
...
def extract_shapekeys_to_keep(
    context, hg_body, apply_armature=True
) -> "tuple[list, dict]":
