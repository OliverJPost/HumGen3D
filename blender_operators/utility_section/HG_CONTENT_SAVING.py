...
from ...blender_backend.HG_PCOLL import refresh_pcoll  # type: ignore
from ...blender_backend.HG_SHAPEKEY_CALCULATOR import (
    build_distance_dict,
...
from ...blender_operators.creation_phase.HG_LENGTH import (
    apply_armature,
    correct_origin,
)
...
class HG_OT_SAVE_SHAPEKEY(bpy.types.Operator, Content_Saving_Operator):
    ...
    def execute(self, context):
        ...
        sks_to_remove = [
            sk
            for sk in sk_obj.data.shape_keys.key_blocks
            if sk.name not in data and sk.name != "Basis"
        ]
...
class HG_OT_SAVEPRESET(bpy.types.Operator, Content_Saving_Operator):
    ...
    def add_material_data(self, preset_data, hg_body, hg_eyes) -> dict:
        ...
        for nodename, input_name in nodename_dict.items():
            nodegroup_dict[nodename] = {
                input_name: nodes[nodename].inputs[input_name].default_value,
            }
