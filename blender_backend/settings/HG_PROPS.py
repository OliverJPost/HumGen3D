...
from ...blender_backend.HG_CALLBACK import tab_change_update
from ...blender_backend.HG_PCOLL import get_pcoll_enum_items, refresh_pcoll
from ...blender_operators.common.HG_COMMON_FUNC import make_path_absolute
from ...blender_operators.creation_phase.HG_BODY import scale_bones
from ...blender_operators.creation_phase.HG_HAIR import (
    load_hair,
    update_hair_shader_type,
)
from ...blender_operators.creation_phase.HG_LENGTH import update_length
from ...blender_operators.creation_phase.HG_MATERIAL import (
    load_textures,
    toggle_sss,
    toggle_underwear,
)
from ...blender_operators.finalize_phase.HG_CLOTHING import load_pattern
from ...blender_operators.finalize_phase.HG_CLOTHING_LOAD import load_outfit
from ...blender_operators.finalize_phase.HG_EXPRESSION import load_expression
from ...blender_operators.finalize_phase.HG_POSE import load_pose
...
class HG_SETTINGS(bpy.types.PropertyGroup):
    ...
    ####### batch mode ###########
    generate_amount: IntProperty(name="Amount", default=10, min=1, max=100)
    batch_progress: IntProperty(
    ...
    batch_clothing_inside: BoolProperty(
        name="Inside",
        default=True,
        update=lambda a, b: HG_BATCH_UILIST.batch_uilist_refresh(
            a, b, "outfits"
        ),
    )
    ...
    batch_average_height_cm_male: IntProperty(
        name="Male [cm]", default=175, min=160, max=190
    )
    batch_average_height_cm_female: IntProperty(
    ...
    batch_apply_armature_modifier: BoolProperty(
        name="Apply armature modifier", default=True
    )
    batch_remove_clothing_solidify: BoolProperty(
        name="Remove clothing solidify", default=True
    )
    batch_remove_clothing_subdiv: BoolProperty(
        name="Remove clothing subdiv", default=True
    )
    batch_apply_clothing_geometry_masks: BoolProperty(
    ...
    thumb_render_path: StringProperty(default="", subtype="DIR_PATH")
